...
사용법:
    python3 quality_gate.py 제안서.pptx [--names 금지명단.txt] [--palette "1F3864,8FAADC,..."]
...
BANNED = ["최고", "완벽", "혁신적", "획기적", "100%", "무중단", "완전 자동화",
          "위험 제로", "유일한", "업계 1위"]
...
def run(path: Path, names: list[str], palette: set[str]) -> list[str]:
    fails: list[str] = []
    blocks = extract_blocks(path)
    for i, block in enumerate(blocks, 1):
        loc = f"슬라이드 {i}" if len(blocks) > 1 else "본문"
        has_evidence = any(m in block for m in EVIDENCE_MARKS)
        for w in BANNED:
            if w in block and not has_evidence:
...
def main() -> int:
    ...
    fails = run(a.file, names, palette)
    if fails:
